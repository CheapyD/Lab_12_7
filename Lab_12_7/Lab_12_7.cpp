...
void AddElem(StElem*& first, StElem*& last, string prizv, double AveMark, int stipend);
...
int main()
{
	...
	int N;
	double x;
	cout << "Введіть кількість студентів в списку: "; cin >> N;
	cout << endl;
	...
	for (int i = 0; i < 3; i++)
		Print(first, i);
	cout << endl;
	cout << "Пошук студентів з вказаним середнім балом:" << endl;
	cout << "Середній бал: "; cin >> x;
	for (int i = 0; i < 3; i++)
		Find(first, i, x);
	cout << endl;
	cout << "Видалення зі списку студентів, що не мають стипендії:" << endl;
	Scanning(first, last);
	for (int i = 0; i < 3; i++)
		Print(first, i);
...
void CreateList(StElem*& first, StElem*& last, int N)
{
	...
	{
		cin.sync();
		cout << "Прізвище студента: "; cin >> prizv;
		cout << "Середній бал: "; cin >> AveMark;
		do
		{
			cout << "Стипендія: "; cin >> stipend;
		} while (stipend < 0);
		AddElem(first, last, prizv, AveMark, stipend);
		cout << endl << endl;
	}
}
...
void Print(StElem* first, int i)
{
	while (first != NULL)
	{
		if (i == 0)
			...
			cout << "| " << setw(8) << left << first->AveMark << " ";
		else
